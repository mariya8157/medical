...
final AmbulanceRepositoryProvider = Provider(
    (ref) => AmbulanceRepository(firestore: ref.watch(fireStoreProvider)));
...
class AmbulanceRepository {
  ...
  add(AmbulanceModel ambulanceData) {
    ambulance.add(ambulanceData.toMap()).then(
      (value) {
  ...
  streamAmbulance() {
    return ambulance.snapshots().map((event) => event.docs
        .map((e) => AmbulanceModel.fromMap(e.data() as Map<String, dynamic>))
        .toList());
  }
