...
final HsptlRepositoryProvider =
    Provider((ref) => HsptlRepository(firestore: ref.watch(fireStoreProvider)));
...
class HsptlRepository {
  ...
  add(HsptlModel hsptlData) {
    _hsptl.add(hsptlData.toMap()).then(
      (value) {
  ...
  streamHsptl() {
    return _hsptl.snapshots().map((event) => event.docs
        .map((e) => HsptlModel.fromMap(e.data() as Map<String, dynamic>))
        .toList());
  }

  UpdateHsptl(HsptlModel hsptledit) {
    print(_hsptl.doc(hsptledit.id));
    _hsptl.doc(hsptledit.id).update(hsptledit.toMap()).then(
          (value) {},
        );
  }
}
